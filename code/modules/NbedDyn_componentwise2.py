        ...
        class FC_net(torch.nn.Module):
                    ...
                    def get_NonMarkClosure(self, aug_inp):
                        ...
                        for k in range(self.K):
                            ...
                            foo = self.f_nn_nonMark(aug_inp_k)
                            g[:,k] = foo[:,0] # store observed component
                            g[:, self.K + k*self.D: self.K + (k+1)*self.D] = foo[:,1:] # store un-observed components
                        return g
